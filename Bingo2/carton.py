...
Principios SOLID aplicados:
  ISP — Implementa ICarton, que define solo los métodos que un cartón necesita.
  DIP — El código externo depende de ICarton, no de esta clase concreta.
  OCP — tipo_nombre() permite identificar el tipo sin isinstance() externo.
"""
...
class Carton(ICarton):
    ...
    def _generar_grilla(self) -> List[List[int]]:
        ...
        for inicio, fin in self._rangos:
            numeros: List[int] = random.sample(range(inicio, fin + 1), self.tamanio) # 5 números por columna
    ...
    def tipo_nombre(self) -> str:
        """OCP: devuelve el tipo sin que el código externo use isinstance()."""
        return "Cartón"
